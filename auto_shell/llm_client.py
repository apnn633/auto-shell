...
COMMAND_SYSTEM_PROMPT = """你是一个 Linux/Unix 终端命令生成器。

用户输入自然语言描述，你只需要输出一条可直接执行的 shell 命令。

当前环境：
{context}

严格规则：
1. 只输出命令本身，不要输出任何解释、说明、问候语
2. 不要输出 markdown 代码块标记（```）
3. 不要输出 "这是命令"、"您可以使用" 等废话
4. 命令必须是单行、可直接在终端执行的
5. 调用 run_shell_command 工具，传入 command 参数

示例：
用户: 查看当前目录
你: 调用 run_shell_command({{"command": "ls -la"}})

用户: 查找大文件
你: 调用 run_shell_command({{"command": "find . -type f -size +100M"}})
"""
...
# 解释性输出工具 schema（用于 AI 模式下标记非命令输出）
_TOOL_RETURN_EXPLANATION = {
    "type": "function",
    "function": {
        "name": "return_explanation",
        "description": "当无法生成命令时，返回解释性文字（如需要更多信息、请求澄清等）",
        "parameters": {
            "type": "object",
            "properties": {
                "message": {
                    "type": "string",
                    "description": "要向用户展示的解释内容"
                }
            },
            "required": ["message"]
        }
...
class LLMClient:
    ...
    async def generate_command(
        ...
        # 根据配置决定是否启用 AI 智能检测模式
        smart_mode = getattr(config.shell, 'smart_detect_mode', 'regex')
        if smart_mode == 'ai':
            # AI 模式：提供两个工具，让 AI 自己判断
            tools = [_TOOL_RUN_COMMAND, _TOOL_RETURN_EXPLANATION]
            tool_choice = "auto"
        else:
            # 正则模式：强制使用命令工具
        ...
        try:
            logger.info(f"Calling LLM API ({config.llm.model}) with tool_call (smart_mode={smart_mode})...")
                    ...
                    try:
                        ...
                        if tool_name == "run_shell_command":
                            ...
                            if command:
                                ...
                                return command
                        elif tool_name == "return_explanation":
                            # AI 选择返回解释而非命令
                            message = args.get("message", "").strip()
                            if message:
                                # 用特殊标记包裹，让插件识别
                                result = _json.dumps({"type": "explanation", "message": message}, ensure_ascii=False)
                                logger.info(f"AI returned explanation: {message[:50]}...")
                                return result
